/**
 * As contas das calculadoras da trilha — puras, e nos DOIS lados.
 *
 * O QUE ISTO CONSERTA (Auditoria 21/09, DES-03). Estas dezoito funções moravam
 * atrás de dezoito rotas HTTP, e nenhuma toca rede, banco ou IA: são 650 linhas
 * de matemática pura. Cada clique numa calculadora virava ida e volta ao
 * servidor — cerca de 300ms na produção, e mais de dez segundos quando o plano
 * grátis do Render tinha deixado o serviço dormir.
 *
 * Esperar dez segundos por uma multiplicação faz a pessoa concluir que o site
 * está quebrado, e ela estaria certa: a conta podia acontecer no aparelho dela,
 * na hora, inclusive sem internet.
 *
 * Mora em `shared/` porque roda nos dois lados. As rotas continuam existindo
 * sobre ESTAS MESMAS funções — uma implementação só, que é o que garante que a
 * tela e a API nunca divirjam.
 *
 * O CONTRATO. Cada função devolve `{ ok, payload }`. O `payload` é exatamente o
 * corpo que a rota respondia antes, campo por campo — inclusive os extras que
 * alguns erros carregam (`signal: "neutral"`). Foi transcrição mecânica, não
 * reescrita: a rota virou `r.ok ? res.json(r.payload) : res.status(422).json(r.payload)`.
 *
 * A validação vem junto porque é parte da conta: um conjunto de probabilidades
...
export function ev1(corpo: unknown): Resultado {
  const { outcomes, probabilities } = corpo as { outcomes: number[]; probabilities: number[] };
...
export function houseEdge1(corpo: unknown): Resultado {
  const { decimal_odds } = corpo as { decimal_odds: number[] };
  if (!Array.isArray(decimal_odds) || decimal_odds.length < 2)
    return { ok: false, payload: { error: "Forneça pelo menos 2 odds decimais" } };
  if (decimal_odds.some((o) => o <= 1))
    return { ok: false, payload: { error: "Odds decimais devem ser > 1.0" } };
...
export function bayes1(corpo: unknown): Resultado {
  const { prior, likelihood_given_true: lTrue, likelihood_given_false: lFalse } = corpo as Record<string, number>;
  if (prior <= 0 || prior >= 1) return { ok: false, payload: { error: "Prior deve estar em (0, 1) exclusivo" } };
  if (lTrue < 0 || lTrue > 1 || lFalse < 0 || lFalse > 1)
    return { ok: false, payload: { error: "Likelihoods devem estar em [0, 1]" } };

  const pEvidence = lTrue * prior + lFalse * (1 - prior);
  if (pEvidence === 0) return { ok: false, payload: { error: "P(E) = 0 — evidência impossível dados os parâmetros" } };
...
export function zscore2(corpo: unknown): Resultado {
  const { value, mean, std } = corpo as { value: number; mean: number; std: number };
  if (std <= 0) return { ok: false, payload: { error: "Desvio padrão deve ser > 0" } };
...
export function confidenceInterval2(corpo: unknown): Resultado {
  const { mean, std, n, level = 0.95 } = corpo as { mean: number; std: number; n: number; level?: number };
  if (n < 2) return { ok: false, payload: { error: "n mínimo = 2 para calcular IC" } };
  if (level <= 0 || level >= 1) return { ok: false, payload: { error: "Nível de confiança deve estar em (0, 1)" } };
...
export function correlation2(corpo: unknown): Resultado {
  const { x, y } = corpo as { x: number[]; y: number[] };
  if (!Array.isArray(x) || !Array.isArray(y) || x.length !== y.length)
    return { ok: false, payload: { error: "x e y devem ter o mesmo comprimento" } };
  if (x.length < 4) return { ok: false, payload: { error: "Correlação requer n >= 4 observações" } };
...
export function taylorRule3(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["selic_observed", "ipca_12m", "output_gap_pct"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { selic_observed, ipca_12m, output_gap_pct } = corpo as { selic_observed: number; ipca_12m: number; output_gap_pct: number };
...
export function poisson3(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["home_attack", "home_defense", "away_attack", "away_defense"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { home_attack, home_defense, away_attack, away_defense, league_avg_goals = 1.35, is_home = true } = corpo as {
...
export function elo3(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["rating_a", "rating_b"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { rating_a, rating_b, home_advantage = true } = corpo as { rating_a: number; rating_b: number; home_advantage?: boolean };
...
export function garch3(corpo: unknown): Resultado {
  const { returns } = corpo as { returns: number[] };
  if (!Array.isArray(returns) || returns.length < 30)
    return { ok: false, payload: { error: "Série muito curta para GARCH (mínimo 30 obs)", signal: "neutral" } };
...
export function enso3(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["oni_index"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { oni_index } = corpo as { oni_index: number };
...
export function polling3(corpo: unknown): Resultado {
  const { polls } = corpo as { polls: Array<{ date: string; candidate_a_pct: number; candidate_b_pct: number; sample_size?: number }> };
  if (!polls || polls.length === 0) return { ok: false, payload: { error: "Forneça pelo menos uma pesquisa" } };
...
export function prospect4(corpo: unknown): Resultado {
  const { outcomes, probabilities } = corpo as { outcomes: number[]; probabilities: number[] };
...
export function brier4(corpo: unknown): Resultado {
  const { forecasts, outcomes } = corpo as { forecasts: number[]; outcomes: number[] };
  if (!Array.isArray(forecasts) || forecasts.length < 5) return { ok: false, payload: { error: "Forneça pelo menos 5 previsões" } };
  if (forecasts.some((f) => f < 0 || f > 1)) return { ok: false, payload: { error: "Previsões devem estar em [0, 1]" } };
  if (outcomes.some((o) => o !== 0 && o !== 1)) return { ok: false, payload: { error: "Outcomes devem ser 0 ou 1" } };
...
export function gambler4(corpo: unknown): Resultado {
  const { sequence } = corpo as { sequence: number[] };
  if (!Array.isArray(sequence) || sequence.length < 4)
    return { ok: false, payload: { error: "Sequência mínima: 4 eventos", signal: "neutral" } };
...
export function maturity4(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["brier_skill_score", "overconfidence_idx", "n_sessions", "loss_aversion_ratio"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { brier_skill_score, overconfidence_idx, gambler_fallacy_risk, n_sessions, loss_aversion_ratio } = corpo as {
...
export function divergence5(corpo: unknown): Resultado {
  const faltou = faltamNumeros(corpo, ["model_probability", "market_probability"]);
  if (faltou) return { ok: false, payload: { error: faltou } };
  const { model_probability: mp, market_probability: mkp, model_confidence = 0.5, context = "" } = corpo as {
...
export function ensemble5(corpo: unknown): Resultado {
  const { model_probabilities, model_skill_scores } = corpo as { model_probabilities: Record<string, number>; model_skill_scores: Record<string, number> };
  if (!model_probabilities || !model_skill_scores)
    return { ok: false, payload: { error: "Forneça model_probabilities e model_skill_scores" } };
