 ...
 *
 * Sempre busca o superset (limit=200); os chamadores fatiam/filtram conforme
 ...
 * - Caso contrário: busca o superset (limit=200) e cacheia.
 */
export async function getMarkets<T = Record<string, unknown>>(source: MarketSource): Promise<T[]> {
  ...
  const p = fetch(`/api/${source}/markets?limit=200`)
